...
# area kerja di /tmp (ephemeral) — dibersihkan otomatis oleh yt_tool setelah tiap job
...
try:
    if "febspot_cookies" in st.secrets:
        os.environ["FEBSPOT_COOKIES"] = st.secrets["febspot_cookies"]
...
with st.sidebar:
    ...
    if ids:
        ...
        st.caption("Belum ada akun. Tempel cookies dari extension browser (EditThisCookie / Cookie-Editor) setelah login ke febspot.com, lalu klik Tambahkan.")
