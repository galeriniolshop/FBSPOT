...
def load_accounts():
    """Muat akun dari file persisten. Migrasi otomatis dari febspot_cookies.json lama."""
    if os.path.exists(ACCOUNTS_FILE):
        try:
            d = json.load(open(ACCOUNTS_FILE))
            d.setdefault("active", None)
            d.setdefault("accounts", {})
            return d
        except json.JSONDecodeError:
            pass
    # migrasi akun tunggal lama
    acc = _default_accounts()
    if os.path.exists(FEBSPOT_COOKIES):
        try:
            cks = json.load(open(FEBSPOT_COOKIES))
            aid = "default"
            acc["accounts"][aid] = {"id": aid, "label": "Akun Utama", "channel": FEBSPOT_CHANNEL,
                                    "cookies": normalize_cookies(cks), "snapshot": None,
                                    "created": time.time()}
            acc["active"] = aid
            save_accounts(acc)
        except Exception:
...
def build_feb_description(d):
    """Deskripsi untuk febspot: info sumber + deskripsi YT + hashtags."""
    ...
    if len(desc) > 1500:
        desc = desc[:1500] + " ..."
    return "\n".join(p for p in (src, "", desc, "", hashtags) if p)
        ...
        if target in ("febspot", "both"):
            try:
                feb = febspot_upload(path, title=d.get("title"),
                                     description=build_feb_description(d), log=log,
